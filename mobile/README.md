...
## PC에서 화면 보기 — 모바일 뷰어 (화면 고칠 때 이게 제일 빠름)

1. 루트의 **`실행.bat`** (ERP 서버)
2. 이 폴더의 **`모바일뷰어_실행.bat`** → 브라우저에 폰 모양 뷰어가 열립니다

- 안쪽은 **진짜 앱 그대로**입니다. 로그인하고 돌아다닐 수 있습니다.
- 코드를 고치면 **저절로 갱신**됩니다 (새로고침 버튼도 있음).
- 기기 프리셋(iPhone SE / 14 / Pro Max / 갤럭시 S / iPad mini)으로 크기별 확인.
- 휴대폰용(8081)과 **포트가 달라(8082) 동시에 켜둘 수 있습니다.**

PC에서는 **지문·얼굴 잠금과 화면 캡처 방지가 동작하지 않습니다.** 그 둘은 실제 휴대폰에서만 확인됩니다.
뷰어는 브라우저에서 API를 부르므로 `apps/api/.env`의 `CORS_ORIGINS`에 `http://localhost:8082`가 있어야 합니다(설정 완료).
