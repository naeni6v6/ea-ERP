...
import { CardTitle, ErrorView, Spinner } from '../components/ui';
...
export function ProjectDetailScreen({ route }: Props) {
  ...
  // 미완료(마감 빠른 순) → 완료 순으로 — 웹 상세와 같은 규칙
        ...
        {proj.isLoading ? (
          ...
          >
            ...
            <View style={s.card}>
              ...
              <Text style={[s.name, ft.extrabold, tight]}>{p.name}</Text>
              {!!p.goal && <Text style={s.goal}>{p.goal}</Text>}
              <View style={{ marginTop: 6 }}>
                <Progress value={p.progress} />
              </View>
              <Text style={s.progressMeta}>
                업무 {p.totalTasks}건 중 {p.doneTasks}건 완료 · 남음 {p.totalTasks - p.doneTasks}건
              </Text>
            </View>
            ...
            <View style={s.card}>
              ...
              <View style={{ gap: 8, marginTop: 4 }}>
                <Info label="기간" value={`${sd(p.startDate)} ~ ${sd(p.planEndDate)}`} numeric />
                <Info label="담당자" value={p.owner?.name ?? '—'} />
                <Info label="주관부서" value={p.leadDepartment?.name ?? '—'} />
                <Info label="사업유형" value={p.businessType?.name ?? '—'} />
                <Info label="참여부서" value={(p.departments ?? []).map((d) => d.department.name).join(', ') || '—'} />
                {!!p.members?.length && <Info label="참여자" value={p.members.map((m) => m.user.name).join(', ')} />}
              </View>
            </View>
                ...
                {([['수주금액', p.contractAmount], ['예상매출', p.expectedRevenue], ['실행예산', p.budgetAmount]] as const).map(
                  ([label, v]) => (
                    <View key={label} style={s.moneyCell}>
                      ...
                      <Text style={[s.moneyVal, ft.extrabold, numFont, tight]}>{compact(v)}</Text>
            ...
            <View style={s.card}>
              <CardTitle
                right={
                  <Text style={[{ fontSize: 12.5, color: colors.inkFaint }, numFont]}>
                    {p.doneTasks}/{p.totalTasks} 완료
                  </Text>
                }
              >
                업무
              ...
              <View style={{ flexDirection: 'row', gap: 8, marginTop: 4 }}>
                ...
                <Pressable
                  ...
                  style={[s.addBtn, (busy || !newTitle.trim()) && { opacity: 0.45 }]}
                >
              ...
              {tasks.isLoading ? (
                ...
                sortedTasks.map((t) => {
                  ...
                  const late = !t.isDone && !!due && due < today;
                  return (
                    <View key={t.id} style={s.taskRow}>
                      <Pressable onPress={() => toggleTask(t)} disabled={busy} style={s.checkWrap} hitSlop={8}>
                        <View style={[s.checkbox, t.isDone && s.checkboxOn]}>
                          {t.isDone && <Ionicons name="checkmark" size={15} color="#fff" />}
                        </View>
                      </Pressable>
                      <View style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 2 }}>
                          ...
                          {!!due && (
                            <Text style={[s.taskMeta, numFont, late && { color: colors.neg, ...ft.semibold }]}>
                              · 마감 {sd(due)}
                              {late ? ' 지남' : ''}
                            </Text>
                          )}
...
const sd = (iso: string | null): string => (iso ? iso.slice(0, 10).replace(/-/g, '.') : '미정');
...
function Info({ label, value, numeric }: { label: string; value: string; numeric?: boolean }) {
  return (
    <View style={{ flexDirection: 'row', gap: 10 }}>
      ...
      <Text style={[{ flex: 1, fontSize: 13.5, color: colors.ink, lineHeight: 19 }, numeric && numFont]}>{value}</Text>
    </View>
  );
}
...
const s = StyleSheet.create({
  ...
  name: { fontSize: 20, color: colors.ink, lineHeight: 26 },
  goal: { fontSize: 13.5, lineHeight: 20, color: colors.inkMute },
  progressMeta: { fontSize: 12.5, color: colors.inkFaint },
  moneyRow: { flexDirection: 'row', gap: 8, marginTop: 4 },
  moneyCell: { flex: 1, backgroundColor: colors.bgSoft, borderRadius: 12, padding: 10, gap: 2 },
  ...
  taskMeta: { fontSize: 12, color: colors.inkFaint },
