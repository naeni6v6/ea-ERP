 ...
 * 컬럼 폭 4px·상단 4px 라운드는 기준선에서 시작, 라인 2px·마커 r4+흰 링.
 */
  ...
  return (
    <div>
      ...
      <div className="relative px-1 pb-2">
        <svg viewBox={`0 0 ${W} ${H}`} className="w-full" role="img" aria-label="월별 매출·영업이익 추이">
          {/* 그리드 + 눈금 */}
          {ticks.map((t) => (
            <g key={t}>
              <line x1={padL} x2={W - padR} y1={y(t)} y2={y(t)} stroke={t === 0 ? BASE : GRID} strokeWidth={1} />
              <text x={padL - 8} y={y(t) + 3.5} textAnchor="end" fontSize={10} fill={TICK_INK} className="font-num">
          ...
          {/* 월 라벨 */}
          {points.map((p, i) => (
            <text key={i} x={cx(i)} y={H - 8} textAnchor="middle" fontSize={10.5} fill={TICK_INK}>
              {p.label}
            </text>
          ))}
