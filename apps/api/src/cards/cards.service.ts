...
export class CardsService {
  ...
  async listCards(u: AuthUser) {
    ...
    const pending = await this.prisma.cardExpense.groupBy({
      by: ['cardId'], where: { cardId: { in: cards.map((c) => c.id) }, status: 'PENDING' }, _count: { _all: true },
    });
    return cards.map((c) => ({ ...c, pendingCount: pending.find((p) => p.cardId === c.id)?._count._all ?? 0 }));
  }
  ...
  async updateCard(u: AuthUser, id: string, d: Partial<CardDto>) {
    ...
    const after = await this.prisma.corporateCard.update({
      where: { id },
      data: { name: d.name, issuer: d.issuer, cardType: d.cardType, last4: d.last4, holderUserId: d.holderUserId, bankAccountId: d.bankAccountId, source: d.source, isActive: d.isActive },
    });
